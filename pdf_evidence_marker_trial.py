...
class TrialManager:
    ...
    初回起動日を %APPDATA%/LeanTechLibrary/trial_evidence_marker.json に
    記録し、残り日数を返す。アンインストールしても記録は残る。
    """
...
# 刻印本体（stamp_evidence_number）と位置検証（verify_stamp）は stamp_core.py へ
# 集約した（有料版と共通実装）。v2.0 で刻印フォントを LTL Evidence Sans
#（同梱TTF・サブセット埋め込み）へ変更。フォント非同梱環境では従来どおり
# PyMuPDF 内蔵の "japan" へ自動フォールバックする。
...
def main():
    ...
    if expired:
        # 期限切れ → ダイアログ表示後にアプリ終了（メインウィンドウは開かない）
        dlg.exec()
        sys.exit(0)
    else:
        # 期限内 → 「閉じて続ける」でメインウィンドウへ。
        # ×ボタン（Rejected）で閉じた場合もアプリ終了。
        result = dlg.exec()
